...
BEGIN_MESSAGE_MAP(CMFCClientDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BTN_SEND, &CMFCClientDlg::OnBnClickedBtnSend)
END_MESSAGE_MAP()
...
void CMFCClientDlg::OnBnClickedBtnSend()
{
	...
	if(TRUE != client.InitClient(TEXT("127.0.0.1"), 5680))
	{
		AfxMessageBox(TEXT("connect failed"));
		return ;
	}

	AfxMessageBox(TEXT("connect success"));

	client.SendFile(strPath);

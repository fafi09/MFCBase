...
BEGIN_MESSAGE_MAP(CMFCServerDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BTN_START, &CMFCServerDlg::OnBnClickedBtnStart)
END_MESSAGE_MAP()
...
void CMFCServerDlg::OnBnClickedBtnStart()
{
	// TODO: 在此添加控件通知处理程序代码

	if(TRUE != m_ServerSocket.InitServer(5680))
	{
		AfxMessageBox(TEXT("server failed"));
		return;
	}

	AfxMessageBox(TEXT("server success"));

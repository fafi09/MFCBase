...
应用程序向导已为您创建了此 ATLCalc1 项目，作为编写动态链接库 (DLL) 
的起点。
...
ATLCalc1.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(ATLCalc1.h)
        GUID 定义			(ATLCalc1_i.c)
        类型库				(ATLCalc1.tlb)
        封送处理代码			(ATLCalc1_p.c 和 
                                           dlldata.c）

ATLCalc1.h
    此文件包含 ATLCalc1.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 ATLCalc1.pch 的预编译头 (PCH) 文件和
...
ATLCalc1ps.vcxproj
    ...
    文件。此进程生成 dlldata.c、ATLCalc1_i.c 和 
    ATLCalc1_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

ATLCalc1ps.vcxproj.filters

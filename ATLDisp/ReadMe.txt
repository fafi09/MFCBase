...
应用程序向导已为您创建了此 ATLDisp 项目，作为编写动态链接库 (DLL) 
的起点。
...
ATLDisp.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(ATLDisp.h)
        GUID 定义			(ATLDisp_i.c)
        类型库				(ATLDisp.tlb)
        封送处理代码			(ATLDisp_p.c 和 
                                           dlldata.c）

ATLDisp.h
    此文件包含 ATLDisp.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 ATLDisp.pch 的预编译头 (PCH) 文件和
...
ATLDispps.vcxproj
    ...
    文件。此进程生成 dlldata.c、ATLDisp_i.c 和 
    ATLDisp_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

ATLDispps.vcxproj.filters
